...
idTempArray는 스코프에서 나갈때 ( {} 에서 나갈때. ) 자동으로 해제되는 배열입니다. 
There is no "cast" operator because these are very unsafe.
여기에는 형변환 연산자가 없는데, 매우 위험하기 때문입니다. 

The template parameter MUST BE POD!
템플릿 매게변수는 반드시 POD여야 합니다! 

(Plain Old Data structure (POD)는 객체지향 프로그래밍(object-oriented programming)의 오브젝트(object)가 아닌, 레코드(record)를 의미하는 말이다. 
PODS라고 쓰기도 하며, passive data structure 라고 쓰기도 한다.
레코드는 변수들의 필드로 이루어진 데이터 구조로, 구조면에서 객체지향 프로그래밍의 오브젝트와 유사하나 객체지향 프로그래밍이 필요로 하는 필수 기능이 빠져 있다.
예를 들어 레코드에서는 변수들의 집합으로서 쓸 수 있으며 포인터를 이용한 함수 사용까지는 가능하나, 
클래스의 상속이라던가 public, private 등의 변수 공개 여부를 지정하는 등 오브젝트로서의 기능이 없다.) - 위키 피디아. 

POD = 단순한 변수들의 나열. C언어 struct 생각하면 될듯?

Compile time asserting POD-ness of the template parameter is complicated due
to our vector classes that need a default constructor(생성자) but are otherwise
considered POD.

컴파일 시간중 POD-Ness한 템플릿 페러미터는 예외로 처리하려고 했습니다만, 
우리의 백터 클래스들은(컨테이너말고, vector3f 같은 것들.) 기본 생성자가 있어서 그렇게 하지 못하였습니다. 
