...
퀵 소트, 힙 소트, 인 셀렉트 소트와 같은 일반화 템플릿된 소트 알고리즘입니다. 
...
name and other times based on say priority). 
이 소트 알고리즘들은 클래스 오퍼레이터들이나, 함수 오버라이드 등을 사용하지 않았습니다. 
왜냐하면, 동일한 오브젝트를 다른 방법과 다른 키들을 기반으로 정렬하는 것이 종종 바람직하기 때문입니다.  

So instead, for each different sort a separate class is implemented with a Compare() function.
그래서 대신에, 각각의 다른 정렬에 대해서 별도의 클래스가 Compare() 함수로 구현됩니다. 
 
This class is derived from one of the classes that implements a sort algorithm. 
이 클래스는 소트 알고리즘을 구현한 클래스들 중에서 파생됩니다. 

The Compare() member function does not only define how objects are sorted, the class
can also store additional data that can be used by the Compare() function. 
Compare() 맴버 함수는 객체 정렬 방법을 정리할 뿐만 아니라, Compare() 함수에서 사용할 수 있는
추가 데이터를 저장할 수도 있습니다. 

This, for instance, allows a list of indices to be sorted where the indices point to objects in an array.
예를 들어, 인덱스 배열을 사용해 색인으로 배열 안에 있는 객체를 정렬할 수 있습니다. 
(추가 데이터 관련 설명. )


The base pointer of the array with objects can be stored on the class
...
/*
================================================
idSort_Quick is a sort template that implements the quick-sort algorithm on an array of objects of the specified data type.
================================================
*/
